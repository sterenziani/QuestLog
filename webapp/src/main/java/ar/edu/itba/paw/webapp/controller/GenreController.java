...
public class GenreController {
	...
	@Path("/{genreId}/games")
	@Produces(value = { MediaType.APPLICATION_JSON })
	public Response getGamesByGenre(@PathParam("genreId") long genreId, @QueryParam("page") @DefaultValue("1") int page, @QueryParam("page_size") @DefaultValue("15") int page_size, @QueryParam("backlog") @DefaultValue("") String backlog)
	{
		final Optional<Genre> maybeGenre = gens.findById(genreId);
		if(!maybeGenre.isPresent())
			return Response.status(Response.Status.NOT_FOUND.getStatusCode()).build();
